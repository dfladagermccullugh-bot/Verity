...
import { generateResumePhrase } from "@/lib/resume-phrase";
import {
  beginInterview,
  submitAnswer,
  type AnswerParadata,
...
/** Client-supplied paradata for an answer; timing is captured in the browser. */
...
export async function startSession(token: string, seedRaw: string): Promise<StartResult> {
  ...
  if (existing) {
    if (existing.status === "complete") return { ok: true, kind: "done" };
  ...
  const [session] = await db
    ...
    .values({
      ...
      seedWarnings: JSON.stringify(seedWarnings(seed)),
      resumePhrase: generateResumePhrase(),
    })
    .returning();
...
export async function answer(
  ...
  if (session.status === "complete") return { ok: true, kind: "done" };

  const pd: AnswerParadata = {
    timeToAnswerMs: paradata.timeToAnswerMs,
    deviceClass: paradata.deviceClass ?? null,
    viewport: paradata.viewport ?? null,
  };
  const result = await submitAnswer(session, value, pd);
  if (result.kind === "question")
    return { ok: true, kind: "question", text: result.text };
  return { ok: true, kind: "done" };
}
